...
public class ValidarInventarioController {
    ...
    @FXML
    private TableView<InventarioDetalle> tblValidar;
    @FXML
    ...
    private void onRegistrar(ActionEvent e) {
        if (!Alerta.confirmar("Confirmar", "¿Registrar este conteo de inventario?")) {
            return;
        }
        try {
            int idEmp = Sesion.getInstance().getEmpleadoActual().getIdUsuario();
            inventarioDAO.registrarConteo(idEmp, detalles);
            Alerta.info("Exito", "Conteo de inventario registrado correctamente.");
        } catch (Exception ex) {
