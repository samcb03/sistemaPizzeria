...
public class EmpleadoDAO implements IEmpleadoDAO {
    ...
    public boolean registrar(Empleado emp, String telefono, String email) throws Exception {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(
                ...
                ps.setString(3, emp.getApellidoMaterno()); ps.setString(4, emp.getCiudad());
                ps.executeUpdate();
                try (ResultSet rk = ps.getGeneratedKeys()) { rk.next(); idUsuario = rk.getInt(1); }
            }
            try (PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO Empleado (idEmpleado,username,contrasena,Rol_idRol) VALUES (?,?,SHA2(?,256),?)")) {
                ps.setInt(1, idUsuario); ps.setString(2, emp.getUsername());
                ps.setString(3, emp.getContrasena()); ps.setInt(4, emp.getRol().getIdRol());
                ps.executeUpdate();
            }
            ...
            conn.rollback(); throw new Exception("Error al registrar empleado: " + e.getMessage(), e);
    ...
    public boolean actualizar(Empleado emp, String telefono, String email) throws Exception {
        ...
        try {
            try (PreparedStatement ps = conn.prepareStatement(
                ...
                ps.setString(1,emp.getNombre()); ps.setString(2,emp.getApellidoPaterno());
                ps.setString(3,emp.getApellidoMaterno()); ps.setString(4,emp.getCiudad());
                ps.setInt(5,emp.getIdUsuario()); ps.executeUpdate();
            }
            try (PreparedStatement ps = conn.prepareStatement(
                    "UPDATE Empleado SET username=?,Rol_idRol=? WHERE idEmpleado=?")) {
                ps.setString(1,emp.getUsername()); ps.setInt(2,emp.getRol().getIdRol());
                ps.setInt(3,emp.getIdUsuario()); ps.executeUpdate();
            }
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM Telefono WHERE Usuario_idUsuario=?")) {
                ps.setInt(1,emp.getIdUsuario()); ps.executeUpdate();
            }
            if (telefono != null && !telefono.isBlank()) {
                try (PreparedStatement ps = conn.prepareStatement("INSERT INTO Telefono VALUES (?,?)")) {
                    ps.setInt(1,emp.getIdUsuario()); ps.setString(2,telefono); ps.executeUpdate();
                }
            }
            ...
            if (email != null && !email.isBlank()) {
                try (PreparedStatement ps = conn.prepareStatement("INSERT INTO Email VALUES (?,?)")) {
                    ps.setInt(1,emp.getIdUsuario()); ps.setString(2,email); ps.executeUpdate();
                }
            }
            conn.commit(); return true;
        } catch (SQLException e) {
            conn.rollback(); throw new Exception("Error al actualizar empleado: " + e.getMessage(), e);
        } finally { conn.setAutoCommit(true); }
    }
    ...
    public List<Empleado> buscarPorNombre(String nombre) throws Exception {
        ...
        try (PreparedStatement ps = getConn().prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) { while (rs.next()) lista.add(mapEmpleado(rs)); }
        }
        return lista;
    }
